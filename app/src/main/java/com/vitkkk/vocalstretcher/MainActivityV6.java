...
/**
 * v0.5.7 workflow:
 * - orange range = final sample boundaries;
 * - green range = ONLY the material that may be lengthened;
 * - audio before/after the green range is copied at original duration;
 * - if the user never changes the green range it defaults to the whole sample,
...
public class MainActivityV6 extends Activity {
    ...
    private Button stretchStartButton;
    private Button stretchEndButton;
    ...
    private int processedStretchStart = -1;
    private int processedStretchEnd = -1;
    ...
    private void buildUi() {
        ...
        title.setText("Vocal Stretcher v0.5.7");
        ...
        hint.setText("Laranja = sample final  •  Verde = trecho que estica  •  1 dedo: cursor  •  2 dedos: zoom/arrastar");
        ...
        LinearLayout stretchRow = horizontalRow();
        stretchStartButton = new Button(this);
        stretchStartButton.setText("Esticar desde aqui");
        stretchStartButton.setOnClickListener(v -> setStretchStart());
        stretchEndButton = new Button(this);
        stretchEndButton.setText("Esticar até aqui");
        stretchEndButton.setOnClickListener(v -> setStretchEnd());
        stretchRow.addView(stretchStartButton, weighted());
        stretchRow.addView(stretchEndButton, weighted());
        root.addView(stretchRow, fullWrap());

        stretchLabel = new TextView(this);
        stretchLabel.setText("Trecho esticado: selecione primeiro o sample");
        stretchLabel.setTextColor(Color.rgb(0, 125, 85));
        stretchLabel.setTextSize(12f);
        stretchLabel.setGravity(Gravity.CENTER_HORIZONTAL);
        stretchLabel.setPadding(0, dp(2), 0, dp(4));
        root.addView(stretchLabel, fullWrap());

        LinearLayout durationRow = horizontalRow();
        TextView durationTitle = new TextView(this);
        durationTitle.setText("Duração FINAL do sample: ");
        ...
        statusLabel.setText("Importe um áudio e faça o corte do sample. Depois ajuste em verde somente o pedaço que deve ser alongado.");
        ...
        worker.execute(() -> {
            try {
                ...
                mainHandler.post(() -> {
                    ...
                    waveformView.setStretchRegion(-1, -1);
                    cutButton.setText("Corte início");
    ...
    private void markCut() {
        ...
        if (!waitingForSecondCut) {
            ...
            statusLabel.setText("Início do sample marcado. Agora leve o cursor ao fim do sample.");
        } else {
            ...
            // Backward-compatible default: if the user does nothing else, the
            // whole selected sample is stretchable exactly as in v0.5.6.
            stretchStart = selectionStart;
            stretchEnd = selectionEnd;
            waveformView.setStretchRegion(stretchStart, stretchEnd);
            ...
            statusLabel.setText("Sample em laranja pronto. Agora, se quiser, marque em verde só a vogal que deve esticar.");
        }
        refreshUi();
    }

    private void setStretchStart() {
        if (!hasFinishedSelection() || busy) return;
        stopPlaybackLabels();
        int maxStart = Math.max(selectionStart, selectionEnd - 1);
        stretchStart = clamp(playheadFrame, selectionStart, maxStart);
        if (stretchEnd <= stretchStart || stretchEnd > selectionEnd) stretchEnd = selectionEnd;
        invalidateProcessedSample();
        waveformView.setStretchRegion(stretchStart, stretchEnd);
        statusLabel.setText("Início verde marcado. Posicione o cursor no fim da vogal e toque em 'Esticar até aqui'.");
        refreshUi();
    }

    private void setStretchEnd() {
        if (!hasFinishedSelection() || busy) return;
        stopPlaybackLabels();
        if (stretchStart < selectionStart || stretchStart >= selectionEnd) stretchStart = selectionStart;
        int candidate = clamp(playheadFrame, selectionStart + 1, selectionEnd);
        if (candidate <= stretchStart) {
            Toast.makeText(this, "O fim da área verde precisa ficar depois do início.", Toast.LENGTH_LONG).show();
            return;
        }
        stretchEnd = candidate;
        invalidateProcessedSample();
        waveformView.setStretchRegion(stretchStart, stretchEnd);
        statusLabel.setText("Área verde pronta. Só esse trecho será alongado; o restante ficará na duração original.");
        refreshUi();
    }
    ...
    private void generateSample(boolean autoPlay) {
        ...
        int prefixFrames = stretchStart - selectionStart;
        int suffixFrames = selectionEnd - stretchEnd;
        int targetStretchFrames = targetFrames - prefixFrames - suffixFrames;

        stopPlaybackLabels();
        setBusy(true, "Alongando somente a região verde e preservando o resto…");
        AudioData base = sourceAudio;
        int sampleStart = selectionStart;
        int sampleEnd = selectionEnd;
        int innerStart = stretchStart;
        int innerEnd = stretchEnd;

        worker.execute(() -> {
            try {
                AudioData stretchedFull = StretchEngineV2.stretchRegion(
                        base, innerStart, innerEnd, targetStretchFrames);
                AudioData sampleOnly = extractSample(stretchedFull, sampleStart, targetFrames);

                mainHandler.post(() -> {
                    processedSample = sampleOnly;
                    processedSourceStart = sampleStart;
                    processedSourceEnd = sampleEnd;
                    processedStretchStart = innerStart;
                    processedStretchEnd = innerEnd;
                    processedTargetFrames = targetFrames;
                    setBusy(false, String.format(Locale.US,
                            "Sample pronto: %.3f s • fora da região verde preservado",
    ...
    private AudioData extractSample(AudioData stretchedFull, int sampleStartFrame, int sampleFrames) {
        ...
        System.arraycopy(stretchedFull.samples, start * stretchedFull.channels, out, 0, out.length);
    ...
    private boolean validateStretch(int targetFrames) {
        ...
        if (!hasStretchRegion()) {
            Toast.makeText(this, "Defina uma área verde válida dentro do sample.", Toast.LENGTH_LONG).show();
            return false;
        }
        int originalSampleFrames = selectionEnd - selectionStart;
        if (targetFrames < originalSampleFrames) {
            Toast.makeText(this, "A duração final deve ser maior ou igual à duração original do sample.", Toast.LENGTH_LONG).show();
            return false;
        }
        int minimumStretch = Math.max(256, (int) Math.round(sourceAudio.sampleRate * 0.050));
        if (stretchEnd - stretchStart < minimumStretch) {
            Toast.makeText(this, "A área verde precisa ter pelo menos ~50 ms para o WORLD analisar a vogal.", Toast.LENGTH_LONG).show();
            return false;
        }
        int preserved = (stretchStart - selectionStart) + (selectionEnd - stretchEnd);
        int targetInner = targetFrames - preserved;
        if (targetInner < stretchEnd - stretchStart) {
            Toast.makeText(this, "Com esses cortes, a duração final ficou curta demais para alongar a área verde.", Toast.LENGTH_LONG).show();
            return false;
        }
        return true;
    }
    ...
    private boolean cacheMatchesCurrentSettings() {
        ...
        return selectionStart == processedSourceStart
                ...
                && stretchStart == processedStretchStart
                && stretchEnd == processedStretchEnd
    ...
    private void exportSampleToFolder(Uri treeUri) {
        ...
        worker.execute(() -> {
            try {
                ...
                String fileName = "VocalSample_v057_" + stamp + ".wav";
    ...
    private void invalidateProcessedSample() {
        ...
        processedStretchStart = -1;
        processedStretchEnd = -1;
    ...
    private boolean hasStretchRegion() {
    ...
    private void refreshUi() {
        ...
        stretchStartButton.setEnabled(selected && !busy);
        stretchEndButton.setEnabled(selected && !busy);
        previewButton.setEnabled(selected && hasStretchRegion() && !busy);
        generateButton.setEnabled(selected && hasStretchRegion() && !busy);
    ...
    private void updateStretchLabel() {
        if (sourceAudio == null || !hasFinishedSelection()) {
            stretchLabel.setText("Trecho verde: selecione primeiro o sample");
            return;
        }
        if (!hasStretchRegion()) {
            stretchLabel.setText("Trecho verde: ainda não definido");
            return;
        }
        double duration = (stretchEnd - stretchStart) / (double) sourceAudio.sampleRate;
        double before = (stretchStart - selectionStart) / (double) sourceAudio.sampleRate;
        double after = (selectionEnd - stretchEnd) / (double) sourceAudio.sampleRate;
        stretchLabel.setText(String.format(Locale.US,
                "Esticar: %s → %s (%.3f s) • preserva %.3f s antes + %.3f s depois",
                formatTime(stretchStart, sourceAudio.sampleRate),
                formatTime(stretchEnd, sourceAudio.sampleRate), duration, before, after));
    }
